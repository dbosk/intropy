...
pronoun" of the variable can be used later in the programme, instead of 
the name that was used in the first definition of the variable. For 
example, if a list has been defined as \verb'my\_list', the list is 
later referenced only as \verb'list'. 
    \item Defining a variable by using another variable students use \verb
'x == y' instead of \verb'x = y', a misconception supposedly 
originating from how the statement is read out loud as \emph{"x equals y
"}.
    \item Writing definitions of variables from left-to-right (\verb'a+b = 
c') instead of right-to-left (\verb'c = a+b'). The same can be seen 
when using functions in the definition of variables, for example 
instead of writing \verb'x=str(input())' the students write \verb'str(x)
 = input()'.
\end{enumerate}
...
the 
students saw. They did not see that the \verb'x' to the left is the 
variable, 
