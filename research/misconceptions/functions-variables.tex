...
\begin{frame}<presentation>
  \begin{block}{Misconception\footcite{%
          Kohn2017VariableEvaluation,%
          Plass2015Variables,%
          Doukakis2007%
        }}
    \begin{itemize}
      \item Students believe that variables can hold an entire algorithm.
      \item They seem to see a variable assignment as a function or a 
...
\begin{frame}
  \begin{block}{Misconception\footcite{Kohn2017VariableEvaluation}}
    \begin{itemize}
      \item If a return statement contains an expression (\enquote{equation}),
        the student believes that the return value will be that expression 
        (function), not the value that it results in.
    \end{itemize}
  \end{block}
\end{frame}

\begin{frame}
From a variation theoretic perspective, we can say two things about this:
\begin{enumerate}
  \item<+> That variables and functions are interconnected and should be 
    treated simultaneously (not \enquote{one thing at a time}), to be able to 
    contrast them \parencite[\cf][Ch~6, pp~167--168]{NCOL}.
...
\begin{frame}
  \begin{assumption}
    Let's assume that the students know variables, functions (or relations or 
    maps) and equations from mathematics and that that's the only prerequisite 
    knowledge.
  \end{assumption}
\end{frame}

From the variation theoretic perspective, one \emph{critial aspect} (dimension 
...
\begin{frame}
  \begin{block}{Critical aspect (dimension of variation): type}<1>
    \begin{description}
      \item[Aspect] Type of object that an identifier refers to.
      \item[Features] Function, data\footnote{%
          In turn divided into data types later on.
        }
    \end{description}
  \end{block}

  \begin{block}{Critical aspect (dimension of variation): statefulness}<2>
    \begin{description}
      \item[Aspect] Statefulness
      \item[Features] Stateful, stateless
    \end{description}
  \end{block}
\end{frame}
...
\begin{description}
  \item [Contrast] We must introduce variation in the aspect (dimension of 
    ...
    \begin{frame}[fragile]
      \only<presentation>{\frametitle{Contrast: Statefulness}}
...
print(f"x = {x}")
    ...
    \item [Generalisation] In the generalisation pattern we want to generalise 
      ...
      \begin{frame}[fragile]
        \only<presentation>{\frametitle{Generalisation: Statefulness}}
      \begin{minipage}[t]{0.45\columnwidth}
...
\begin{frame}
  \begin{block}{Critical aspect (dimension of variation): type}<2>
    \begin{description}
      \item[Aspect] Type of object that an identifier refers to.
      \item[Features] Function, data\footnote{%
          In turn divided into data types later on.
  ...
  \begin{block}{Critical aspect (dimension of variation): statefulness}<1>
    \begin{description}
      \item[Aspect] Statefulness
      \item[Features] Stateful, stateless
    \end{description}
...
\begin{description}
  \item [Contrast] We must introduce variation in the aspect (dimension of 
    ...
    \begin{frame}[fragile]
      \only<presentation>{\frametitle{Contrast: Functionality of functions}}
    \begin{minipage}[t]{0.45\columnwidth}
...
print(f"x(2, 3) = {x(2, 3)}")
  ...
  \item[Generalisation] To generalise, we must keep this aspect invariant (the 
    ...
    \begin{frame}[fragile]
      \only<presentation>{\frametitle{Generalisation: Functionality of 
      functions}}
    \begin{minipage}[t]{0.45\columnwidth}
...
print(greet("Beda", "Hi"))
  ...
  \item[Fusion] Now we'd like to fuse this back with the statefulness of 
    ...
    \begin{frame}[fragile]
      \only<presentation>{\frametitle{Fusion: Functionality of functions, 
      statefulness}}
    \begin{minipage}[t]{0.45\columnwidth}
      \begin{pyblock}[funcG1]
def y(x):
...
\subsubsection{Order or arguments}
...
\begin{frame}[fragile]
  \begin{remark}
    \begin{itemize}
      \item We can return to previous examples for generalization.
      \item The order of arguments were all correct in those.
    \end{itemize}
  \end{remark}
\end{frame}
...
\begin{frame}[fragile]
  \begin{example}[Generalize identifiers in different scopes]
