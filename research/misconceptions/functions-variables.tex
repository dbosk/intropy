...
print(f"x = {x}")
  ...
  \stdoutpythontex[varstateC1][verbatim]
\end{minipage}
\hfill
\begin{minipage}[t]{0.45\columnwidth}
...
print(f"x = {x}")
  ...
  \vspace{0.5em}
  Which yields the following:
  \stdoutpythontex[varstateC2][verbatim]
\end{minipage}
...
differences.
    ...
    other \mintinline{python}{example2}, but that should not be done at 
this 
    stage.)

    \item [Generalisation] In the generalisation pattern we instead let 
the value of \mintinline{python}{x} vary, and keep the critical aspect 
...
\hfill
\begin{minipage}[t]{0.45\columnwidth}
    \begin{minted}[highlightcolor=light-green, highlightlines={4,6}]{python}
        def example():
            a = 2
            b = 5
            x = a + b
