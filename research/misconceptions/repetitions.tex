...
\begin{enumerate}
    \item A misconception about which variable in a loop defined as \verb
'for item in items', that is supposed to be use to extract different 
information.
    \item The misconception that \verb'for i in 100' will iterate a hundred 
times, even though Python require a specified range.
    \item When defining a while-loop, the misconception is that one can 
write \verb'while i <= 100' without initialising \verb'i' beforehand, 
and that \verb'i' will automatically increase with 1 inside the loop. 
\end{enumerate}
