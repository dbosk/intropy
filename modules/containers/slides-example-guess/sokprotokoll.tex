...
påståenden.  De flesta kan du pröva själv genom att köra programmet ---
det är hela poängen med att koden på bilderna är samma kod som körs, och
att körningen i \cref{sec:slump} går att upprepa.  Men några påståenden
går inte att pröva på det sättet:
\begin{enumerate}
  \item att en slumpgenerator utan startvärde utgår från klockan eller
    från operativsystemets slumpkälla, och att den just därför inte
    duger till kryptografi (\cref{sec:slump});
  ...
  \item att en funktion ska ha ett ansvar och att lösningen ska hållas
    ...
    \foreignlanguage{english}{KISS} (\cref{sec:inlasning});
  ...
  \begin{tabular}{@{}p{0.46\linewidth}p{0.44\linewidth}@{}}
    ...
    startvärde?
      & Primärkälla: Pythons dokumentation för modulen
        \mintinline{python}{random}, läst i fulltext 2026-09-03;
        programmet i \cref{sec:slump} visar det dessutom, och körningen
        där går att upprepa. \\
    Beter sig behållarna som materialet beskriver?
      & Primärkälla: Pythons egen dokumentation, läst i fulltext
        2026-09-03. \\
    Ska en funktion ha ett ansvar
    (\foreignlanguage{english}{SRP})?
      & Redan belagt i föreläsningen \emph{Funktioner}, bilaga~B, som
        svarar både på var principen kommer ifrån och hur säker den är:
        etablerad, men vad \enquote{ett ansvar} är förblir en
        bedömning. \\
    Ska lösningen hållas enkel (\foreignlanguage{english}{KISS})?
      & Redan belagt i föreläsningen \emph{Funktioner}, bilaga~C, som
        svarar både på var principen kommer ifrån och om den håller:
        enkelhet som designvärde är etablerad, men den vanliga
        upphovsmannen är fel. \\
...
fungerar\autocite{PythonRandom}.  Dokumentationen för
\mintinline{python}{random} säger både det materialet använder ---
\enquote{The generator's \mintinline{python}{random()} method will
continue to produce the same sequence when the compatible
...
Två principer namnges här utan att beläggas här: det vetenskapliga
underlaget för \foreignlanguage{english}{SRP} --- ett ansvar per
funktion --- finns i föreläsningen \emph{Funktioner}, bilaga~B, som
svarar både på var principen kommer ifrån och hur säker den är, och
underlaget för \foreignlanguage{english}{KISS} --- att hålla lösningen
enkel --- finns i samma föreläsnings bilaga~C, som svarar både på var
principen kommer ifrån och om den håller.  Att kontrast måste upplevas
samtidigt för att en aspekt ska kunna urskiljas är hämtat från Martons
\emph{Necessary
