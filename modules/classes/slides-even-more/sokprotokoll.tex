...
\begin{description}
  ...
  \item[Primärkällan] för var en idé \emph{kommer ifrån}, och för ett
    ...
    och kan man inte det, säger man det.
...
\begin{itemize}
  ...
  \item Sök också efter \emph{invändningar}: lägg till ord som
    ...
    av det påståendet avråder från.  Ett påstående som överlevt en
...
\enquote{föredra}, inte \enquote{undvik}.
...
\cref{sec:komp-diskussion} vad som begränsar svaret och
\cref{sec:komp-slutsats} ger det; \cref{tab:sok-komposition} sist i
...
Patterns} hämtades inte: den är inte indexerad i databaserna och finns
inte öppet tillgänglig, så tillskrivningen vilar på andrahandskällor
och står som sådan i källfilen (se \cref{sec:komp-diskussion}).
...
Sammanlagt gav sökningarna 417 unika poster.  Alla klassades: först med
...
bort av det skälet.  De källor som citeras nedan kunde bara läsas i
...
\enquote{favor object composition over class inheritance}.  Boken
själv kunde inte läsas här --- den är varken öppen eller indexerad ---
så tillskrivningen vilar på andrahandskällor; ingen annan upphovsman
till rådet kom fram i sökningen.
...
structure}.  Ett femtontal ytterligare poster kvalificerar på liknande
sätt --- arvets energieffektivitet, alternativ till arv för flerfaldigt
arv, arv i testkod --- och står som kvalificerande i
\cref{tab:sok-komposition}.

\section{Diskussion och begränsningar}
\label{sec:komp-diskussion}

Tre saker begränsar svaret.  Verifieringen är svag: av de citerade
källorna lästes bara Snyder i fulltext; de fem empiriska studierna är
stängda hos IEEE, Elsevier, Springer och ACM och lästes i
sammanfattning, och boken som rådet tillskrivs kunde inte läsas alls
--- meningen och sidan (s.\ 20) är hämtade ur andrahandskällor och bör
kontrolleras av den som har boken.  Källorna gäller C++, Java och
...
Ja på båda frågorna, med förbehåll.  Rådet att föredra komposition
framför arv kommer från \emph{Design Patterns} såvitt sökningen kan se
och är etablerat i objektorienterad design: ett tjugotal poster bygger
på det, och arvets kostnader --- bruten inkapsling, svårare förståelse,
fler fel --- är belagda \parencite{Snyder1986,WildeHuitt1992,CartwrightShepperd2000}.
Förbehållen kom ur motsökningen: experimenten om arv och underhåll
pekar åt olika håll \parencite{Daly1996,Harrison2000}, den sköra
basklassen ställer sällan till det i praktiken \parencite{Sabane2017},
och arv används flitigt och för det mesta enkelt
\parencite{StevensonWood2018}.  Därför säger materialets tumregel
\enquote{föredra komposition, och använd arv när ni verkligen har en
är-en-relation} --- inte \enquote{undvik arv}.
...
%   known items     : Gamma1994DesignPatterns (a book; not indexed, not
%                     obtained -- attribution via secondary sources, to
%                     verify in the book)
