...
en källa som tidigare var läst i fulltext inte längre gick att komma åt,
och det står därför uttryckligen att den inte kontrollerats om.
...
källorna gick inte att läsa om; minnestiderna stämmer som
...
% Marton2015 pp. 45-47 as verified in the course; MartonPang2006 abstract
% only (ERIC EJ733793), closed at T&F and HKU hub.  Bib keys:
...
materialet är byggt på.  \Textcite[45--47]{Marton2015} visar att en
aspekt bara kan urskiljas om den varierar mot en invariant bakgrund, och
att den variationen måste upplevas samtidigt: \textcquote[46]{Marton2015}{if
you cannot see the greenness of any of three green, though otherwise
different, things, you cannot see what they have in common, either}.
Samma villkor formuleras i den tidskriftsartikel materialets lärarnoter
sedan tidigare hänvisade till: \textcquote{MartonPang2006}{To discern an
aspect, the learner must experience potential alternatives, that is,
variation in a dimension corresponding to that aspect, against the
background of invariance in other aspects of the same object of
learning}.  Det är därför de två läsprogrammen i \cref{sec:lasa-skriva}
visas sida vid sida inför valövningen, och därför körningen i
\cref{sec:json} visar filens text och det inlästa på samma bild.
...
För det första gick en av källorna inte att läsa om.  Studien med
nybörjarförklaringarna hade tidigare kontrollerats i fulltext i kursens
genomgång, i författarens egen kopia på webben; den kopian är nu
tillbakadragen, och förlagets version är stängd.  Citatet ovan bärs
alltså av den tidigare kontrollen, inte av en ny läsning, och det står
så i källfilen.  Påståendet vilar därför i första hand på
antimönsterkatalogen, som lästes om i sin helhet; studien med
förklaringarna tillför att felet gäller just filer, vilket katalogen inte
säger ut.

För det andra är den lärandeteoretiska artikeln läst enbart i
sammanfattning; den är stängd hos förlaget.  Den citeras därför bara för
det sammanfattningen säger rakt ut --- att urskiljning kräver variation
mot invarians --- och det skarpare påståendet om samtidighet bärs av
boken, vars sidor är kontrollerade i kursen.
...
Ja, missuppfattningen är belagd, med förbehållet att en av de två
källorna inte gick att läsa om och att ingen av dem mäter frekvensen i
den här kursens population.  Att data som läses in inte omvandlas till
tal är ett katalogiserat antimönster i
nybörjarkod\autocite{Bosse2021Antipatterns}, och att felet gäller just
det som läses från filer och terminal står i studenters egna
förklaringar\autocite{GuoMarkelZhang2020}.  Att kontrasterna visas
samtidigt följer av variationsteorins villkor för
urskiljning\autocite[45--47]{Marton2015}, som artikeln materialet
tidigare hänvisade till formulerar på samma sätt i sin
sammanfattning\autocite{MartonPang2006}.  Beläggen används till det de
räcker till: att välja \emph{vilken} fråga som ställs innan tolkningen
visas, inte till att säga åt studenterna vad de tror.
...
samma avsnitt: \mintinline{python}{f.read(size)} \textcquote{PythonTutorialFiles}{returns
it as a string (in text mode) or bytes object (in binary mode)}.
...
Materialet säger att textläget \enquote{oftast} använder UTF-8; det gör
det inte av sig självt.  Referensen säger att
...
to use a different encoding}.  Materialet säger därför i
