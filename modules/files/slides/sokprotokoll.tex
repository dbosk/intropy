...
när filen saknas ser du själv genom att köra det.  Men några av
...
systematiska genomgång av nybörjares missuppfattningar
\parencite{SooriBosk2026}, och de övriga påståendena besvaras av
dokument man vet finns och slår upp direkt ---
\emph{kända dokument}.
...
dokument}.  Både \cref{app:dokumentation} och \cref{app:format}
innehåller sådana fall --- dokumentationen som rekommenderar
\mintinline{python}{with} säger också att standardkodningen beror på
datorn, och RFC:n som beskriver CSV säger i sin inledning att den inte
är någon standard.
...
du inget sådant ställe stöder källan inte påståendet.  Räkna aldrig en
källa du inte läst: \cref{app:missuppfattningar} innehåller ett fall där
...
Svaret skrivs ut tillsammans med det som talar emot.  Tre av bilagorna
svarar \enquote{ja, med förbehåll} och den fjärde delar upp svaret
...
storleksordningar, inte som mätvärden för just din dator; dokumentationen
säger det materialet säger, men standardkodningen beror på datorn; och
CSV är mycket riktigt vanligt men ingen standard, medan JSON är det.
...
stället för att CSV kallas standard rakt av.  Varje kapitel slutar
...
inom ett pågående forskningsarbete \parencite{SooriBosk2026}.  Den
genomgången söktes ämnesdrivet i flera databaser --- Semantic Scholar,
OpenAlex, DBLP, arXiv, Web of Science, IEEE Xplore och Scopus --- på
frågan vilka missuppfattningar nybörjare har i grundläggande
programmering, och träffarna sorterades först maskinellt och sedan för
hand.  Källorna för lärandeteorin är de
...
gäller just data som läses från fil eller terminal står i en samling
nybörjarförklaringar, drygt 16\,000 stycken insamlade under tre år på en
webbplats för programvisualisering:
...
and comparison, respectively}.  Det senare är precis vad frågan i
\cref{sec:egna} är
...
därför de två programmen i \cref{sec:oppna} står i en och samma övning och
får sina körningar på samma bild, därför de två läsprogrammen i
\cref{sec:lasa-skriva} visas tillsammans inför valövningen, därför de två
programmen i \cref{sec:egna} skiljer sig i bara ett \mintinline{python}{int},
och därför körningen i \cref{sec:json} visar filens text och det inlästa på
samma bild.
...
För det första är en av källorna inte längre säkert tillgänglig.
Studien med nybörjarförklaringarna kontrollerades i fulltext i
författarens egen kopia på webben.  Dagen efter att kursens andra
föreläsningar hämtat kopian svarar den adressen med ett meddelande om
att artikeln inte längre är tillgänglig, och förlagets version är
stängd.
Författarens arbetskopia --- samma fil --- fanns kvar under en annan
adress när detta skrevs, och den är inte återkallad hos förlaget;
citatet i \cref{sec:miss-resultat} är läst om i den sparade kopian.
Studien tillför att felet gäller just data som läses från filer, vilket
antimönsterkatalogen inte säger ut.
...
fulltext i författarens eget exemplar.  Artikeln ställer
samtidighetskravet själv, som en av fyra nödvändiga betingelser för
perceptuellt lärande, och hänvisar för det till Marton och Tsui (2004);
det skarpare påståendet bärs därför av både artikeln och boken.
...
Ingen egen sökning gjordes (\cref{sec:miss-metod}), så kapitlet har
varken sökfrågor eller träfflista; att ställa den tvåsidiga frågan om
just den här missuppfattningen, och redovisa den, är det första som
återstår.  Två källor har dessutom var sin lucka.  Samlingen av
nybörjarförklaringar är inte längre öppet tillgänglig
(\cref{sec:miss-diskussion}): citatet är kontrollerat i en sparad
kopia, men en läsare utan den kopian kan inte göra om kontrollen, och
det som skulle rätta till det är förlagets utgåva via ett bibliotek.
Och den bok som artikeln om lärandets betingelser själv hänvisar till
för villkoret om samtidighet --- Marton och Tsui, 2004 --- är inte läst
här; en läsning av den skulle visa om villkoret formuleras lika starkt
i sitt ursprung som i den artikel som citerar
det\autocite[199]{MartonPang2006}.

För ett säkrare svar saknas två slags undersökningar.  Den ena är en
räkning: hur ofta studenter i den här kursen faktiskt jämför strängar
med tal när de läser ur en fil.  Källorna säger att felet finns och
återkommer, inte hur vanligt det är här, och kursens egna frågor är
redan formulerade så att de kan användas till en sådan räkning.  Den
andra är ett experiment om utformningen: att de två läsprogrammen visas
sida vid sida följer av en teoretisk betingelse, inte av ett resultat
om just de programmen, och en jämförelse mellan att visa dem samtidigt
och efter varandra skulle pröva den.  Visar räkningen att felet är
ovanligt här, kan frågan i \cref{sec:egna} bli kortare; visar
experimentet ingen skillnad, står programparen kvar men bilagan slutar
hävda att samtidigheten är nödvändig.
...
webben\autocite{ArpaciDusseau2025OSTEP}.  Boken valdes framför de
...
\section{Fortsatt arbete}
\label{sec:minne-fortsatt}

Svaret vilar på en enda lärobok.  De klassiska läroböckerna i
datorarkitektur, främst Hennessy och Pattersons, vilkas tabeller över
åtkomsttider är den vanliga källan till sådana siffror, gick inte att
öppna i fulltext och är därför inte lästa här.  Ingen litteratursökning
gjordes heller: frågan behandlades som ett känt faktum, vilket är
rimligt för storleksordningar men gör att kapitlet inte vet något om
nyare mätningar.  Och den disktid som kapitlet citerar är ett antagande
i ett räkneexempel, inte en mätning (\cref{sec:minne-diskussion}).

Vad som skulle ge ett säkrare svar är en mätning på de maskiner
studenterna faktiskt använder.  Ett litet program som läser samma mängd
data ur primärminnet och ur en fil, kört i kursens egen miljö, skulle
ge siffror som är både aktuella och kontrollerbara av den som läser ---
och det vore ett program studenterna kan köra själva.  Skillnaden kan
mycket väl visa sig mindre än läroboken anger, eftersom snabba SSD:er
ligger närmare mikrosekunder än millisekunder; blir det resultatet kan
materialet ange två intervall för sekundärminnet i stället för ett, och
stämmer läroboksvärdena kan siffrorna stå kvar med en mätning som stöd
i stället för ett antagande.
...
\paragraph{Läsa och skriva.}  Ja, på alla fem punkterna.
\mintinline{python}{write} lägger inte till något.  Om anropet
\mintinline{python}|f.write(string)| säger handledningen att det
\textcquote{PythonTutorialFiles}{writes the contents of string to the
file, returning the number of characters written}.  Raden
...
(whatever locale.getencoding() returns)}, och handledningen att UTF-8 är
\textcquote{PythonTutorialFiles}{the modern de-facto standard} och att
\mintinline{python}|encoding="utf-8"| därför rekommenderas
\textcquote{PythonTutorialFiles}{unless you know that you need to use a
different encoding}.  Materialet säger därför numera i
...
\section{Fortsatt arbete}
\label{sec:dok-fortsatt}

Kontrollen gäller en utgåva av dokumentationen, hämtad en enda dag
(\cref{sec:dok-diskussion}).  Ingen jämförelse mellan Python-utgåvor
gjordes, så kapitlet vet inte vilka av påståendena som ändrats med
tiden och vilka som stått still.  Påståendena kontrollerades mot
dokumentationen, inte genom att köra kod; tre av dem körs numera ändå i
materialet självt --- \cref{sec:oppna} kör de två programmen som visar
att \mintinline{python}{with} stänger filen även när ett särfall
inträffar, \cref{sec:saknas} kör den saknade filen och \cref{sec:typer}
läser råa byte som text --- men de körningarna gjordes för
undervisningens skull och ingår inte i den här kontrollen.
Standardkodningen är den punkt där dokumentationen själv är i rörelse
--- rekommendationen att alltid ange kodning är ett tecken på det ---
och vilken förändring som är på gång har inte följts upp här.

Det som skulle göra svaret hållbart över tid är ingen studie utan en
kontroll som går att köra om: materialets egna program prövar redan tre
av påståendena, och det som saknas är dels program för de övriga --- att
\mintinline{python}{write} inte lägger till något, att en läst rad
behåller sin radbrytning --- dels att köra dem mot varje Python-utgåva
kursen stöder, så att kontrollen säger till när något slutar gälla.  En öppen
fråga av annat slag är om exempelprogrammen borde ange kodning: att
skriva \mintinline{python}|encoding="utf-8"| i varje anrop är
tydligare men längre, och vilket som hjälper en nybörjare mest är inte
undersökt.  Visar en sådan undersökning att den utskrivna kodningen
inte kostar begriplighet, bör exemplen ange den.
...
egna utsagor, och begränsningen i det redovisas i
\cref{sec:format-diskussion}.  Ingen litteratursökning gjordes.
...
reading and writing the data from the programmer}, och föreskriver hur
filen ska öppnas: \textcquote{PythonCSV}{If csvfile is a file object, it
should be opened with} \mintinline{python}|newline=''|.
...
Nej för CSV och ja för JSON.  CSV är ett mycket vanligt format enligt
...
kallar därför JSON \enquote{en riktig internetstandard}, och säger om CSV
att RFC:n som beskriver det är informativ, inte en internetstandard, och
att varianter förekommer.  CSV införs som \enquote{ett format som redan
finns}, inte som ett standardformat.


\section{Fortsatt arbete}
\label{sec:format-fortsatt}

Frågan om status är avgjord av dokumenten själva, men frågan om
utbredning är det inte: den vilar på två utsagor av dem som beskriver
formatet, och någon sökning efter en mätning av hur vanliga olika
filformat är gjordes aldrig (\cref{sec:format-diskussion}).  Den
sökningen är det första som återstår.  Vidare lästes bara de två
RFC:erna och de två modulsidorna: Pythons dokumentation säger att
modulen \mintinline{python}{json} följer den föregående utgåvan av
JSON-specifikationen, och vad som skiljer utgåvorna åt är inte
kontrollerat här.  Inte heller är det kontrollerat hur väl verkliga
CSV-filer följer RFC~4180 --- dokumentationen säger att skillnader
förekommer, men kapitlet har ingen mätning av hur stora de är.

För ett säkrare svar behövs alltså två mätningar.  Den ena är en
räkning av formaten i öppna datakällor av det slag kursens övningar
använder, som skulle ersätta ordet \enquote{vanligast} med en siffra.
Den andra är en granskning av ett stort antal CSV-filer mot RFC~4180,
som skulle säga hur ofta specialfallen --- kommatecken, citattecken och
radbrytningar i fält --- faktiskt förekommer.  Visar den granskningen
att avvikelserna är vanliga, blir argumentet för modulen
\mintinline{python}{csv} starkare än det är i \cref{sec:csv}; visar
räkningen att JSON tagit över den sortens data, bör materialet ge JSON
mer plats än formatet får i dag.
