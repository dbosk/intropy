...
\begin{restatable}{lo}{FilesMoreLOValidation}\label{FilesMoreLOValidation}%
  Validera att filer existerar och hantera \mintinline{python}{FileNotFoundError}
  på lämpligt sätt.
\end{restatable}
\begin{restatable}{lo}{FilesMoreLODictProcessing}\label{FilesMoreLODictProcessing}%
  Bearbeta textfiler med dictionaries för räkning och analys
  (t.ex.\ ordfrekvens, bokstavsfrekvens).
\end{restatable}
\begin{restatable}{lo}{FilesMoreLOComposition}\label{FilesMoreLOComposition}%
  Komponera program genom att använda funktioner från flera moduler
  tillsammans.
\end{restatable}
\begin{restatable}{lo}{FilesMoreLOTransformation}\label{FilesMoreLOTransformation}%
  Transformera filinnehåll genom att läsa från en fil, bearbeta data,
  och skriva resultat till en annan fil eller samma fil.
\end{restatable}
\begin{restatable}{lo}{FilesMoreLOCSVOutput}\label{FilesMoreLOCSVOutput}%
