...
\ltnote{%
  Relevanta lärandemål: \cref{FilesMoreLOValidation}
...
\begin{frame}
  \begin{remark}[Tre sätt att hantera filfel]
    Exemplen ovan visar tre huvudsakliga strategier:
    \begin{description}
      \item[Krascha] Låt programmet avbrytas. Enklast, men dålig användarupplevelse.
      \item[Fånga] Använd \mintinline{python}{try-except} för att fånga felet och
        hantera det efter att det uppstått.
      \item[Förebygg] Kontrollera att filen finns \emph{innan} vi försöker öppna
        den --- ger bättre användarupplevelse.
    \end{description}
  \end{remark}
\end{frame}

\ltnote{%
  \textbf{Variationsmönster}: Kontrast + Generalisering

  \textbf{Vad som varierar}: Felhanteringsstrategi (krascha vs fånga vs förebygga).

  \textbf{Vad som hålls invariant}: Problemet (filen kanske inte finns).

  \textbf{Pedagogisk sekvens}: Exemplen visades först (krasch, try-except,
  validering), sedan generaliseras de tre strategierna i en remark.
  Studenter erfar variationen genom kodexempel innan mönstret artikuleras
  \parencite{MartonPang2006}.
}
...
\ltnote{%
  Relevanta lärandemål: \cref{FilesMoreLODictProcessing}
...
\ltnote{%
  Relevanta lärandemål: \cref{FilesMoreLOCSVOutput}
...
\ltnote{%
  ...
  \texttt{rövare.py} $\to$ analysera översatt text. Ingen modul \enquote{vet} om den
...
\ltnote{%
  Relevanta lärandemål: \cref{FilesMoreLOComposition}
...
\ltnote{%
  Relevanta lärandemål: \cref{FilesMoreLOTransformation}
