...
att prova sig fram till.  Det första slaget är påståenden om vad
\emph{forskningen} säger om hur människor lär sig
...
\begin{table}[htbp]
  ...
  \caption{Påståendena i den här övningen som frågor, och var svaret
    står.  \enquote{Bilaga} avser bilagorna i det namngivna
    dokumentets anteckningar, inte i det här dokumentet.}
  ...
  \begin{tabular}{@{}p{0.50\linewidth}p{0.42\linewidth}@{}}
    ...
    Lönar det sig att försöka lösa uppgiften innan man läser
      lösningsförslaget? & \emph{Övning: Funktioner och variabler},
      bilaga~B \\
    Glömmer nybörjare att ge slingvariabeln ett värde före slingan? &
    ...
    Är spårning för hand det som avslöjar var felet sitter? &
      Ingen --- se \cref{sec:ovn-fortsatt} \\
    Uppfattas en praktik som frivillig när materialet självt överger
      den? & Ingen --- se \cref{sec:ovn-fortsatt} \\
    \bottomrule
  \end{tabular}
\end{table}
...
Några av den här övningens påståenden saknar underlag, och det är värt
att säga rakt ut vilka.
...
sitter.  Flera uppgifter och ett lärandemål bygger på det.  Vad kursens
...
Det fjärde rör hur materialet självt är skrivet och inte vad du ska
göra.  Varje program i övningen har sitt huvudprogram i en funktion,
som materialet har lärt ut sedan föreläsningen \emph{Funktioner}, och
skälet som anges är att ett material som faller tillbaka på ett rakt
skript lär ut att praktiken var frivillig.  Det är ett påstående om
verkan, och inget av kursens material belägger det.

För dem alla gäller samma sak: de är rimliga, de är vanliga i
...
%                       SinhaKapur2021.  Each carries the originating
%                       deck's provenance block verbatim plus a
%                       "FOUND-VIA (here)" line naming the deck and
%                       appendix that backs it.
%   abstract-level    : Eckert2022Loops (IEEE Xplore closed)
%   open claims       : the four in sec:ovn-fortsatt, marked in
