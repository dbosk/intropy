...
\begin{enumerate}
  ...
  \item de fem missuppfattningar lärarnoterna namnger --- att utskrift
    når skärmen i samma stund den skrivs, att
    ...
    metodnamn lämnar över metoden, och att en ny bindning ersätter den
    förra --- och att de är \emph{vanliga};
...
arbetsgången.  De två första påståendena är redan prövade i tidigare
föreläsningar och övningar, och \cref{tab:graphics-pastaenden} anger
var; det tredje är en premiss med källa i lärarnoternas källfil; de två
sista är ännu inte prövade, och \cref{sec:graphics-fortsatt} säger vad
som återstår.
Arbetsgången beskrivs utförligare, och genomförd på flera påståenden, i
bilagorna till föreläsningarna \emph{Algoritmiskt tänkande} och
\emph{Upprepningar}.

\begin{table}[htbp]
  \centering
  \caption{Övningens påståenden som frågor, och var svaren finns.  De
    två första är besvarade i bilagorna till tidigare föreläsningar och
    övningar, det tredje är
    en premiss som hela kursen delar, och de två sista är öppna och
    listade i \cref{sec:graphics-fortsatt}.}
  ...
  \begin{tabular}{@{}p{0.48\linewidth}p{0.42\linewidth}@{}}
    ...
    Hjälper det att försöka själv före förklaringen?
      & Ja, med måttlig effekt, och effekten kommer av jämförelsen och
        inte av dröjsmålet: belagt i \emph{Övning: Funktioner och
        variabler}, bilaga~B \\
...
hjälper att försöka själv först gäller, enligt bilaga~B i \emph{Övning:
Funktioner och variabler}, med måttlig effekt och med undantag för yngre
...
Den här bilagan redovisar ingen egen sökning.  Av övningens fem
påståenden är två återanvända från tidigare bilagor, ett är
kursens variationsteoretiska premiss, och två är öppna.  De två öppna är
följande, och var och en är markerad i källfilen där den görs:
\begin{enumerate}
  \item Att de fem missuppfattningarna är vanliga: att nybörjare
    uppfattar utskrift som \enquote{en rad i taget} och därför inte
    väntar sig buffring (\cref{sec:term-clock}), att de läser
    \mintinline{python}{mainloop} som \enquote{starta programmet} i
    stället för \enquote{lämna över kontrollen} (\cref{sec:clock}), att
    de läser av komponenten i stället för att lagra värdet
    (\cref{sec:timer}), att de förväxlar att anropa en
    \foreignlanguage{english}{callback} med att lämna över den
    (\cref{sec:colors}), och att de väntar sig att en ny bindning
