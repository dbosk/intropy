...
\usepackage{import}
% beamer loads graphicx itself; memoir does not.
\usepackage{graphicx}
...
% sessions (didactic's \runpython[transcript]), so the solutions never
% hand-copy their own output: pythontex runs from ltxobj (the outputdir),
% and workingdir=.. moves it up to the deck directory so examples/...
% resolves.  Every program here reads input, so the runs are transcripts
% with the answers fed in with stdin.
...
% margin notes empties the pool and dies with "Too many unprocessed
