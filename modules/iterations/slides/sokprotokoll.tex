...
Materialet undervisar mot fem konkreta missuppfattningar: att det som
...
\mintinline{python}{in} (\cref{sec:generalisering}); att en
\mintinline{python}{while}-slinga tar slut även om räknaren varken sätts
före slingan eller ändras i kroppen (\cref{sec:while}); och att en
upprepning gör samma nytta som en förgrening när handlingen bara ska
utföras en gång (\cref{sec:upprepa-eller-valja}).  Frågan är: \emph{är det
...
\paragraph{Kod intill slingan.}  Den bäst belagda av de fem.
...
iterate over}.  Det är precis de två felen \cref{sec:generalisering}
respektive \cref{sec:while} tar upp.
...
styrvariabel aldrig ändras: \textcquote[111]{Bosse2021Antipatterns}{The
...
att den måste \emph{ändras} i kroppen med Bosse med fleras antimönster RS2,
som säger just det.  Ingen av källorna får bära hela påståendet ensam.
...
Ja, fyra av de fem påståendena är belagda i primärkällor, och det femte sägs
inte längre.  Kod intill slingan, kravet på något iterbart efter
\mintinline{python}{in}, den oinitierade räknaren, styrvariabeln som aldrig
...
\textcite{Nachtigall2020} försökte upprepa effekten utanför STEM och
misslyckades: \textcquote{Nachtigall2020}{the findings did not replicate the
PF effect on learning in a non-STEM domain}.  Till det kommer ett
...
därför står de två programmen bredvid varandra i stället för på var sin
bild.
