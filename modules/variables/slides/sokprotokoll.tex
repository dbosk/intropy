...
\emph{du} har läst stället den pekar på.  Källorna till modulens
...
\emph{visa} en kontrast, inte som påstående om vad just du tror.  Det
...
det är du själv som måste stå till svars för innehållet.
...
Innan den här sökningen stod det i \cref{sec:kommentarer} att du ska
...
IEEE Xplore, arXiv, Web of Science och Scopus.  Semantic Scholar avvisade
sin nyckel och saknas därför helt.  IEEE Xplores dygnskvot tog slut efter de
...
databas.  Varje fråga står där i den booleska form som skickades till
OpenAlex, IEEE Xplore och arXiv; Web of Science fick samma sträng som
...
\begin{longtable}{@{}ll%
  ...
  \caption{Sökfrågor och antal träffar per databas för sökningen om
    PEP~8.  Bokstaven i första kolumnen säger vilken sorts fråga raden
    är: S en stödfråga, C en motfråga och T en kort omfråga ställd för
    DBLP:s skull.  Bokstaven i andra kolumnen är delfrågan: (a)~namn,
    (b)~utseende, (c)~konvention, (d)~PEP~8 och nybörjare,
    (e)~motsökning.  Databaskolumnerna är OpenAlex (OA; förkortningen
    står för databasens namn, inte för
    \foreignlanguage{english}{open access}), DBLP, IEEE Xplore (IEEE),
    arXiv, Web of Science (WoS) och Scopus.  Högst 25 träffar hämtades
    per databas och fråga, så en 25 betyder \enquote{minst 25}, och ett
    tankstreck betyder att frågan inte ställdes i den databasen --- IEEE
    Xplores dygnskvot tog slut före de korta frågorna.}%
...
precis den avgränsning den här modulen gör: du ska veta att guiden finns
och följa den, inte lära dig den utantill.  Och att ens namnregeln är
