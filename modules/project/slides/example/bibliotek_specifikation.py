...
def skriv_ut_böcker(titel_dictionary):
    '''
...
def meny(titel_dictionary, författare_dictionary):
    '''
    Visar och låter användaren välja vad som ska göras, anropar en funktion motsvarande användarens val
    IN: titel_dictionary (dict, sträng: Bok), författare_dictionary (dict, sträng: Bok)
    UT: N/A
    '''
    pass

def läs_böcker_från_fil():
    '''
...
def skriv_böcker_till_fil(titel_dictionary):
    '''
    skriver böcker till fil med hjälp av en av våra 2 dictionaries
    skriver över allt som har stått i filen innan (ifall en bok har blivit borttagen)
    IN: titel_dictionary (dict, sträng: Bok)
    UT: N/A
    '''
    pass
...
def main():
    '''
    1. läser in böcker från fil
    2. presenterar programmet
